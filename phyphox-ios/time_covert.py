...
def convert_csv(input_csv_path):
    ...
    # df = df[['Time (s)'] + [col for col in df.columns if col != 'Time (s)']]
...
# def delete_converted_files(data_folders):
#     for data_folder in data_folders:
#         data_path = os.path.join('phyphox-ios', 'data', data_folder)
#         for file in os.listdir(data_path):
...
# if __name__ == "__main__":
#     # Define the folders containing data
#     data_folders = []
...
#     # Delete converted_data.csv files
#     delete_converted_files(data_folders)
